...
export default function ProductCard({ 
  ...
  // PROTECCIÓN CONTRA EL ERROR "12"
  if (p?.esDesayuno && opcionesDesayuno?.[p.id]?.agrandarJugo) {
    precioMostrar += 1000;
  }

  return (
    <div style={{ background: 'white', borderRadius: '28px', overflow: 'hidden', minHeight: '600px', display: 'flex', flexDirection: 'column', border: `2px solid #f0f0f0`, boxShadow: '0 8px 15px rgba(0,0,0,0.03)' }}>
      ...
      <div style={{ padding: '20px', flexGrow: 1, display: 'flex', flexDirection: 'column' }}>
        <div style={{ minHeight: '90px' }}>
          ...
          <p style={{ color: MONO_NARANJA, fontWeight: '900', fontSize: '24px', margin: '0' }}>${(precioMostrar || 0).toLocaleString('es-CO')}</p>
        </div>
        ...
        <div style={{ flexGrow: 1, display: 'flex', flexDirection: 'column', gap: '10px', justifyContent: 'center' }}>
          {p?.esDesayuno && (
            <div style={{ background: MONO_AMARILLO, padding: '12px', borderRadius: '15px', display: 'flex', flexDirection: 'column', gap: '8px' }}>
              ...
              <label style={{ display: 'flex', alignItems: 'center', gap: '8px', cursor: 'pointer', padding: '5px', borderRadius: '8px', fontSize: '13px', fontWeight: 'bold', color: MONO_NARANJA }}>
          ...
          {p?.esArroz && (
            <div style={{ background: MONO_AMARILLO, padding: '12px', borderRadius: '15px' }}>
              ...
              <div style={{ display: 'flex', gap: '10px', marginTop: '10px' }}>
                <label style={{ fontSize: '13px' }}><input type="checkbox" checked={conHuevo} onChange={(e) => setConHuevo(e.target.checked)} /> +Huevo</label>
                <label style={{ fontSize: '13px' }}><input type="checkbox" checked={conQueso} onChange={(e) => setConQueso(e.target.checked)} /> +Queso</label>
              </div>
            </div>
          )}
        ...
        <div style={{ marginTop: 'auto', paddingTop: '15px' }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '10px' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
              <button onClick={() => restarCantidad(p?.id)} style={{ width: '32px', height: '32px', borderRadius: '50%', background: MONO_AMARILLO, border: 'none' }}>-</button>
              <span style={{ fontWeight: '900', fontSize: '18px' }}>{cantidades[p?.id] || 1}</span>
              <button onClick={() => sumarCantidad(p?.id)} style={{ width: '32px', height: '32px', borderRadius: '50%', background: MONO_NARANJA, color: 'white', border: 'none' }}>+</button>
            </div>
          </div>
          <button onClick={() => agregarAlCarrito(p)} disabled={!tiendaAbierta || todoAgotado} style={{ width: '100%', background: todoAgotado ? '#ccc' : MONO_NARANJA, color: 'white', border: 'none', padding: '15px', borderRadius: '15px', fontWeight: 'bold', cursor: 'pointer' }}>Añadir</button>
        </div>
      </div>
    </div>
  );
