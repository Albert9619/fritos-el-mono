...
const productosBase = [
  ...
  { id: "7", nombre: "Palitos de Queso Costeño", precio: 2000, categoria: "Fritos", imagen: "/palitos.jpg", disponible: true },
...
export default function App() {
  ...
  // 💾 PERSISTENCIA DEL PEDIDO (LOCALSTORAGE)
  useEffect(() => {
  ...
  const agregarAlCarrito = (itemFinal) => {
    setPedido([...pedido, { ...itemFinal, idUnico: Date.now() }]);
    setNotificacion(`¡${itemFinal.cantidad}x ${itemFinal.nombre} añadido! 🥟`);
  ...
  const enviarWhatsApp = () => {
    ...
    const totalPedido = pedido.reduce((acc, i) => acc + i.subtotal, 0);
  ...
  return (
    <div style={{fontFamily: 'sans-serif', backgroundColor: '#fffcf5', minHeight: '100vh', paddingBottom: '120px'}}>
      ...
      {notificacion && (
        <div style={{position:'fixed', top:'20px', left:'50%', transform:'translateX(-50%)', background: MONO_VERDE, color:'white', padding:'15px 30px', borderRadius:'50px', zIndex: 10000, fontWeight:'bold', boxShadow: '0 5px 15px rgba(0,0,0,0.2)'}}>
          {notificacion}
        </div>
      )}
      ...
      {pedido.length > 0 && (
        <div 
          ...
          style={{
            ...
            fontSize: '32px', boxShadow: '0 10px 30px rgba(249, 115, 22, 0.5)', 
            zIndex: 9999, cursor: 'pointer', transition: '0.3s'
          }}
        >
          🛒
          <span style={{
            position:'absolute', top:'0', right:'0', background:'red', color: 'white',
            fontSize:'14px', minWidth:'22px', height:'22px', borderRadius:'50%',
