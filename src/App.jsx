...
export default function App() {
  ...
  useEffect(() => {
    const verificarEstado = () => {
      ...
      const estaEnHorario = tiempoActualEnMinutos >= 360 && tiempoActualEnMinutos < 1350;
  ...
  const agregarAlCarrito = (p) => {
   ...
   // 3. CONSTRUCCIÓN DEL DETALLE (CORREGIDO Y COMPLETADO)
    let textoDetalle = "";
    ...
    // AGREGAMOS ESTO: Lógica para Bebidas Calientes (Tinto, Café, Chocolate, Aromáticas)
    else if (p.config && p.categoria === "Bebidas") {
      const partes = [];
      if (sel.sabor) partes.push(sel.sabor); // Para elegir el sabor de la aromática
    ...
    // AGREGAMOS ESTO: Lógica para Fritos, Arroces y Bebidas Frías (Coca-Cola, Jugos)
    else {
      const detallesFritos = [];
      if (sel.sabor) detallesFritos.push(sel.sabor);
      if (sel.tamano) detallesFritos.push(sel.tamano.nombre);
      if (sel.extras && sel.extras.length > 0) detallesFritos.push(`Ex: ${sel.extras.join(', ')}`);
      
      textoDetalle = detallesFritos.join(" ").trim();
    }

    
    
   // 4. GUARDAR EN EL PEDIDO (Corregido para que "aprenda")
  const agregarAlCarrito = (p) => {
    // ... (Aquí va la lógica de validación y cálculo del bloque 3)

    // 🧠 NUEVO: Guardar en el historial para que la predicción funcione
    const sel = selecciones[p.id] || {};
    if (sel.sabor) {
    ...
    setPedido([...pedido, { 
      idUnico: Date.now() + Math.random(), // ID mucho más seguro
      nombre: p.nombre, 
      cantidad: (cantidades[p.id] || 1), 
      subtotal: (subtotal || 0), // viene del bloque 3
      detalle: (textoDetalle || "") // viene del bloque 3
    }]);
  ...
  // PREDICCIÓN CORREGIDA (Para incluir aromáticas)
  const predecirSeleccion = (p) => {
    let saborElegido = null;
    // Buscamos en opciones, sabores O sabores dentro de config (aromáticas)
    const listaOpciones = p.opciones || p.sabores || p.config?.sabores;

    if (listaOpciones) {
      // Normalizamos la lista (porque algunas son objetos y otras son texto simple)
      ...
      // Intentamos buscar el favorito, si no, el primero disponible
    ...
    if (p.tamanos && p.tamanos.length > 0) {
      ...
      tamanoElegido = disponibles[0] || p.tamanos[0];
    }
  ...
  // LA FUNCIÓN DE WHATSAPP ESTÁ PERFECTA, SOLO ASEGÚRATE DE QUE "totalSinDom" ESTÉ DEFINIDO
  ...
  // CÁLCULO DE TOTALES (Mantenlo fuera de las funciones)
  ...
  {/* ... (Todo el inicio del bloque isAdmin está perfecto) */}

{(p.opciones || p.sabores || (p.config && (p.config.sabores || p.config.acompanamiento))) && (
  <div style={{marginTop:'10px', display:'flex', gap:'10px', flexWrap:'wrap', background:'#f8fafc', padding:'10px', borderRadius:'15px'}}>
    ...
    {/* 1. Opciones normales (Fritos) */}
    {(p.opciones || p.sabores || []).map((opt, idx) => (
      <div key={`opt-${idx}`} style={{display:'flex', alignItems:'center', gap:'5px', background:'white', padding:'5px 10px', borderRadius:'10px', border:'1px solid #eee'}}>
    ...
    {/* 2. Opciones dentro de CONFIG (Aromáticas/Desayunos) */}
    {p.config && p.config.sabores && p.config.sabores.map((saborNombre, idx) => (
      <div key={`conf-${idx}`} style={{display:'flex', alignItems:'center', gap:'5px', background:'white', padding:'5px 10px', borderRadius:'10px', border:'1px solid #eee'}}>
        <small>{saborNombre} (Sabor)</small>
        {/* Aquí podrías añadir un switch si decides que los sabores de aromáticas también se pueden agotar */}
        <span style={{fontSize:'10px', color:MONO_NARANJA}}>✓ Activo</span>
      </div>
    ))}
  </div>
)}

{/* ... (La parte de los tamaños y los inputs de precio está EXCELENTE) */}
    ...
    <div style={{ fontFamily: 'system-ui, sans-serif', backgroundColor: '#fffcf5', minHeight: '100vh', paddingBottom: '120px', color: MONO_TEXTO }}>
      ...
      <div style={{display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(310px, 1fr))', gap: '30px', padding: '0 20px', maxWidth:'1300px', margin:'0 auto'}}>
        {productosMostrar.filter(p => p.categoria === categoriaActiva).map(p => {
            ...
            // 💰 Lógica de precio base (Tamaño o Precio normal)
            const pBase = (p.tamanos && p.tamanos.length > 0) 
              ? (sel.tamano?.precio || p.tamanos[0]?.precio || 0) 
              : (p.precio || 0);

            // ➕ Suma de extras y adicionales
            const costoExtras = (sel.extras || []).reduce((acc, n) => {
              const exObj = extrasMostrar.find(e => e.nombre === n);
              return acc + (exObj ? exObj.precio : 0);
            }, 0);
            ...
            return (
              <div key={p.id} className="card-mono" style={{background: 'white', borderRadius: '40px', padding: '20px', display: 'flex', flexDirection: 'column', boxShadow: '0 10px 20px rgba(0,0,0,0.02)', border:'1px solid #f1f5f9', opacity: p.disponible === false ? 0.6 : 1}}>
                ...
                {/* 3. LÓGICA ESPECIAL PARA DESAYUNOS (Blindada) */}
                {p.categoria === "Desayunos" && p.config && (
                  <div style={{display: 'flex', flexDirection: 'column', gap: '10px', marginBottom: '10px'}}>
                    ...
                    <label style={{fontSize: '12px', fontWeight: 'bold', display: 'flex', alignItems: 'center', gap: '5px'}}>
                      <input type="checkbox" checked={sel.agrandar || false} onChange={(e) => setSelecciones({...selecciones, [p.id]: {...sel, agrandar: e.target.checked}})} /> 🥤 Agrandar Jugo (+1k)
                    </label>
                  </div>
                )}
                  ...
                  <div style={{display: 'flex', gap: '5px', flexWrap: 'wrap', marginBottom: '10px'}}>
                    {extrasMostrar.map(ex => (
                      <button key={ex.id} onClick={() => {
                        ...
                        setSelecciones({...selecciones, [p.id]: {...sel, extras: nuevos}});
                      }} className={`opcion-btn ${sel.extras?.includes(ex.nombre) ? 'active' : ''}`}>
                        {ex.nombre} {ex.precio > 0 ? `(+$${ex.precio/1000}k)` : ''}
                      </button>
                    ))}
                  </div>
                )}
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '10px', marginBottom: '10px' }}>
                    {p.config.sabores && (
                      <div style={{ display: 'flex', gap: '5px', flexWrap: 'wrap' }}>
                        {p.config.sabores.map(s => <button key={s} onClick={() => setSelecciones({...selecciones, [p.id]: {...sel, sabor: s}})} className={`opcion-btn ${sel.sabor === s ? 'active' : ''}`}>{s}</button>)}
                      </div>
                    )}
                    {p.config.leche && (
                      <div style={{ display: 'flex', gap: '5px' }}>
                        {p.config.leche.map(l => <button key={l} onClick={() => setSelecciones({...selecciones, [p.id]: {...sel, leche: l}})} className={`opcion-btn ${sel.leche === l ? 'active' : ''}`}>{l}</button>)}
                      </div>
                    )}
                    {p.config.azucar && (
                      <div style={{ display: 'flex', gap: '5px' }}>
                        {p.config.azucar.map(a => <button key={a} onClick={() => setSelecciones({...selecciones, [p.id]: {...sel, azucar: a}})} className={`opcion-btn ${sel.azucar === a ? 'active' : ''}`}>{a}</button>)}
                      </div>
                    )}
                  </div>
                )}

                {/* CONTROLES FINALES (Cantidad y Botón) */}
                <div style={{marginTop: 'auto'}}>
                  <div style={{display:'flex', alignItems:'center', justifyContent:'center', gap:'15px', marginBottom:'15px', background:'#f8fafc', padding:'10px', borderRadius:'15px'}}>
                    <button onClick={() => setCantidades({...cantidades, [p.id]: Math.max(1, cant - 1)})} style={{width:'35px', height:'35px', borderRadius:'50%', border:'none', cursor:'pointer'}}>-</button>
                    <span style={{fontWeight:'900', fontSize:'18px'}}>{cant}</span>
                    <button onClick={() => setCantidades({...cantidades, [p.id]: cant + 1})} style={{width:'35px', height:'35px', borderRadius:'50%', border:'none', background:MONO_NARANJA, color:'white', cursor:'pointer'}}>+</button>
                  </div>
                  <button 
...
3. Ajustar el detalle para WhatsA
    ...
    if (p.categoria === "Desayunos") {
      ...
      // Para fritos normales
      const saborTxt = sel.sabor || '';
      const tamanoTxt = sel.tamano?.nombre || '';
      const extrasTxt = sel.extras?.length > 0 ? 'Ex: ' + sel.extras.join(', ') : '';
      det = `${saborTxt} ${tamanoTxt} ${extrasTxt}`.trim();
    }

    // --- 🛒 GUARDAR EN EL PEDIDO ---
    setPedido([...pedido, { idUnico: Date.now(), nombre: p.nombre, cantidad: cant, subtotal, detalle: det }]);
   ...
   {/* 1. SECCIÓN DE JUGOS (Solo para desayunos) */}
    {p.config?.jugos && (
      <div style={{display: 'flex', gap: '5px', marginBottom: '10px'}}>
        {p.config.jugos.map(j => (
          <button 
            key={j} 
            onClick={() => setSelecciones({...selecciones, [p.id]: {...sel, jugo: j}})} 
            className={`opcion-btn ${sel.jugo === j ? 'active' : ''}`}
          >
            {j}
          </button>
        ))}
    ...
    {/* 2. SECCIÓN DE LECHE Y AZÚCAR (Solo para Bebidas Calientes) */}
    {p.config && p.categoria === "Bebidas" && (
      <div style={{ display: 'flex', flexDirection: 'column', gap: '10px', marginBottom: '10px' }}>
        ...
        {p.config.leche && (
          <div style={{ display: 'flex', gap: '5px' }}>
            {p.config.leche.map(l => (
              <button 
                key={l} 
                onClick={() => setSelecciones({...selecciones, [p.id]: { ...sel, leche: l }})} 
        ...
        {p.config.azucar && (
          <div style={{ display: 'flex', gap: '5px' }}>
            {p.config.azucar.map(a => (
              <button 
                key={a} 
                onClick={() => setSelecciones({...selecciones, [p.id]: { ...sel, azucar: a }})} 
                className={`opcion-btn ${sel.azucar === a ? 'active' : ''}`}
              >
                {a}
              </button>
            ))}
...
{/* --- SECCIÓN: CANTIDAD Y BOTÓN AÑADIR (Final del mapa de productos) --- */}
                <div style={{display:'flex', alignItems:'center', justifyContent:'center', gap:'15px', marginBottom:'15px', background:'#f8fafc', padding:'10px', borderRadius:'15px'}}>
                   <button onClick={() => setCantidades({...cantidades, [p.id]: Math.max(1, cant - 1)})} style={{width:'40px', height:'40px', borderRadius:'50%', border:'none', cursor:'pointer'}}>-</button>
                   <span style={{fontWeight:'900', fontSize:'20px'}}>{cant}</span>
                   <button onClick={() => setCantidades({...cantidades, [p.id]: cant + 1})} style={{width:'40px', height:'40px', borderRadius:'50%', border:'none', background:MONO_NARANJA, color:'white', cursor:'pointer'}}>+</button>
                </div>
                <button 
                  onClick={() => agregarAlCarrito(p)} 
                  disabled={!tiendaAbierta || p.disponible === false} 
                  style={{width:'100%', background: (tiendaAbierta && p.disponible !== false) ? MONO_NARANJA : '#ccc', color:'white', border:'none', padding:'18px', borderRadius:'20px', fontWeight:'900', cursor:'pointer'}}
                >
                  {p.disponible === false ? 'AGOTADO' : 'Añadir 🛒'}
                </button>
              </div>
      ...
      {pedido.length > 0 && (
        <div id="carrito_seccion" style={{ maxWidth: '750px', margin: '50px auto 100px', background: 'white', padding: '40px', borderRadius: '40px', border: `5px solid ${MONO_NARANJA}`, boxShadow: '0 20px 45px rgba(0,0,0,0.1)' }}>
          ...
          {/* 🌟 VENTA SUGERIDA RÁPIDA (Solo si no es mesa y falta para el envío gratis) */}
                ...
                {sugeridos.map(sug => {
                  ...
                  return (
                    <button key={sug.id} onClick={() => {
                        // Mejoramos la construcción del detalle para el añadido rápido
                        let detalleRapido = `${prediccion.sabor || ''} ${prediccion.tamano?.nombre || ''}`.trim();
                        setPedido([...pedido, { 
                          idUnico: Date.now() + Math.random(), // ID mucho más seguro
                          nombre: sug.nombre, 
                          cantidad: 1, 
          ...
          <div style={{ marginTop: '30px', background: '#f8fafc', padding: '25px', borderRadius: '25px' }}>
            ...
            <div style={{display: 'flex', gap: '10px', flexWrap: 'wrap'}}>
              {salsasMostrar.filter(s => s.disponible).map(s => (
                <button 
                  key={s.id} 
                  onClick={() => setSalsasElegidas(salsasElegidas.includes(s.nombre) ? salsasElegidas.filter(x => x !== s.nombre) : [...salsasElegidas, s.nombre])} 
                  className={`salsa-chip ${salsasElegidas.includes(s.nombre) ? 'active' : ''}`}
                >
                  {s.nombre}
                </button>
              ))}
