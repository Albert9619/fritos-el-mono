        ...
        <div style={{maxWidth:'850px', margin:'0 auto'}}>
          ...
          {["Fritos", "Arroces", "Bebidas", "Desayunos"].map(cat => (
            <div key={cat} style={{background:'white', padding:'25px', borderRadius:'25px', marginBottom:'25px', boxShadow:'0 4px 6px rgba(0,0,0,0.05)'}}>
              ...
              {/* 🟢 SWITCHES PARA ADICIONES DE ARROZ */}
              {cat === "Arroces" && (
                <div style={{marginTop:'20px', borderTop:'2px dashed #eee', paddingTop:'15px'}}>
                  <h4 style={{margin:'0 0 10px 0', color: MONO_NARANJA}}>Gestionar Adiciones:</h4>
                  {extrasMostrar.map(ex => (
                    <div key={ex.id} style={{display:'flex', justifyContent:'space-between', marginBottom:'10px', alignItems:'center', background:'#fdf7f2', padding:'8px 15px', borderRadius:'12px'}}>
                      <small style={{fontWeight:'bold'}}>{ex.nombre}</small>
                      <div style={{display:'flex', gap:'10px', alignItems:'center'}}>
                        $ <input type="number" defaultValue={ex.precio} onBlur={(e) => guardarCambio("extrasArroz", ex.id, { precio: Number(e.target.value) })} style={{width:'70px', padding:'4px', borderRadius:'8px', border:'1px solid #ccc'}} />
          ...
          <div style={{background:'white', padding:'25px', borderRadius:'25px', boxShadow:'0 4px 6px rgba(0,0,0,0.05)', marginBottom:'25px'}}>
            <h2 style={{borderBottom:'2px solid #eee', paddingBottom:'10px', marginBottom:'15px'}}>Salsas Disponibles 🧂</h2>
