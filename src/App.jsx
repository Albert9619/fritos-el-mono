...
export default function App() {
  ...
  useEffect(() => {
    ...
    onSnapshot(collection(db, "salsas"), (s) => setSalsasFB(s.docs.map(d => ({ id: d.id, ...d.data() }))));
  ...
  useEffect(() => { localStorage.setItem("pedido_mono_storage", JSON.stringify(pedido)); }, [pedido]);
  ...
  const agregarAlCarrito = (p) => {
    ...
    if (p.sabores && !sel.sabor) return alert("Elige el sabor del jugo");
    if (p.tamanos && !sel.tamano) return alert("Elige el tamaño");
    if (p.categoria === "Desayunos") {
        if (!sel.acompanamiento || !sel.jugo || (p.id === "d1" && !sel.huevos) || (p.id === "d2" && !sel.proteina)) return alert("Completa las opciones del desayuno");
    }
    ...
    if (p.categoria === "Desayunos") detalle += `(${sel.acompanamiento}, ${sel.jugo}${sel.agrandar ? ' Gr' : ''})`;
      ...
      <div style={{padding: '20px', background: '#f8fafc', minHeight: '100vh', fontFamily: 'sans-serif'}}>
        <div style={{maxWidth:'850px', margin:'0 auto'}}>
          ...
          {["Fritos", "Arroces", "Bebidas", "Desayunos"].map(cat => (
            <div key={cat} style={{background:'white', padding:'20px', borderRadius:'25px', marginBottom:'20px', boxShadow:'0 4px 6px rgba(0,0,0,0.05)'}}>
              ...
              {productosMostrar.filter(p => p.categoria === cat).map(p => (
                <div key={p.id} style={{padding:'15px 0', borderBottom:'1px solid #f1f5f9'}}>
                  ...
                  {/* PRECIO E INPUTS PARA PRODUCTOS NORMALES */}
                  {!p.tamanos && cat !== "Bebidas" && (
                    <div style={{marginTop:'10px'}}>$ <input type="number" defaultValue={p.precio} onBlur={(e) => guardarCambio("productos", p.id, { precio: Number(e.target.value) })} style={{width:'80px', padding:'5px', borderRadius:'8px', border:'1px solid #ddd'}} /></div>
                  )}
                  {/* INPUTS PARA TAMAÑOS (COKE, PONY, JUGOS) */}
                  {p.tamanos && (
                    <div style={{display:'grid', gap:'10px', marginTop:'10px'}}>
                      {p.tamanos.map((t, idx) => (
                        <div key={idx} style={{display:'flex', justifyContent:'space-between', background:'#f8fafc', padding:'8px', borderRadius:'12px'}}>
                          ...
                          <div style={{display:'flex', gap:'10px'}}>
                            ...
                            }} style={{width:'70px', border:'1px solid #ccc', borderRadius:'5px'}} />
                ...
                <div style={{marginTop:'20px', borderTop:'1px dashed #ddd', paddingTop:'15px'}}>
                  ...
                  {extrasMostrar.map(ex => (
                    <div key={ex.id} style={{display:'flex', justifyContent:'space-between', marginBottom:'10px'}}>
                      ...
                      <div style={{display:'flex', gap:'10px'}}>
                        <input type="number" defaultValue={ex.precio} onBlur={(e) => guardarCambio("extrasArroz", ex.id, { precio: Number(e.target.value) })} style={{width:'70px'}} />
          ...
          <div style={{background:'white', padding:'20px', borderRadius:'25px'}}>
            <h2 style={{borderBottom:'2px solid #eee'}}>Disponibilidad Salsas</h2>
  ...
  return (
    <div style={{fontFamily: 'system-ui, sans-serif', backgroundColor: '#fffcf5', minHeight: '100vh', paddingBottom: '120px', color: MONO_TEXTO}}>
      <style>{`.opcion-btn { padding: 10px; border-radius: 12px; border: 1px solid #ddd; background: white; cursor: pointer; font-weight: bold; } .opcion-btn.active { background: ${MONO_NARANJA}; color: white; } .salsa-chip { padding: 12px; border-radius: 15px; border: 2px solid #eee; background: white; cursor: pointer; font-weight: bold; } .salsa-chip.active { border-color: ${MONO_NARANJA}; background: #fff7ed; color: ${MONO_NARANJA}; }`}</style>
      ...
      <div style={{display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(310px, 1fr))', gap: '30px', padding: '0 20px', maxWidth:'1300px', margin:'0 auto'}}>
        {productosMostrar.filter(p => p.categoria === categoriaActiva).map(p => {
            ...
            const extrasP = (sel.extras || []).reduce((acc, id) => acc + (extrasMostrar.find(e => e.id === id)?.precio || 0), 0);
            ...
            return (
              <div key={p.id} className="card-mono" style={{background: 'white', borderRadius: '40px', padding: '20px', display: 'flex', flexDirection: 'column', boxShadow: '0 10px 20px rgba(0,0,0,0.02)', border:'1px solid #f1f5f9', position: 'relative'}}>
                ...
                {/* OPCIONES DE FRITOS Y JUGOS */}
                {(p.opciones || p.sabores) && (
                  <select onChange={(e) => setSelecciones({...selecciones, [p.id]: {...sel, sabor: e.target.value}})} style={{width:'100%', padding:'12px', borderRadius:'15px', marginBottom:'10px'}}>
                ...
                {p.tamanos && (
                  <select onChange={(e) => setSelecciones({...selecciones, [p.id]: {...sel, tamano: p.tamanos.find(t => t.nombre === e.target.value)}})} style={{width:'100%', padding:'12px', borderRadius:'15px', marginBottom:'10px'}}>
                ...
                {p.categoria === "Arroces" && (
                  <div style={{background: '#fef3c7', padding: '10px', borderRadius: '15px', marginBottom: '10px'}}>
                     {extrasMostrar.map(e => (
                       <label key={e.id} style={{display:'block', fontSize:'14px', opacity: e.disponible ? 1 : 0.3}}>
                ...
                {p.categoria === "Desayunos" && (
                    <div style={{display: 'flex', flexDirection: 'column', gap: '8px', marginBottom: '10px'}}>
                        ...
                        <label style={{fontSize: '13px', fontWeight: 'bold', background: '#f0fdf4', padding: '8px', borderRadius: '10px'}}><input type="checkbox" onChange={(e) => setSelecciones({...selecciones, [p.id]: {...sel, agrandar: e.target.checked}})} /> 🥤 Agrandar Jugo (+1k)</label>
                        <div style={{fontSize: '22px', color: MONO_VERDE, fontWeight: '900', textAlign: 'center'}}>🧀 ¡INCLUYE QUESO!</div>
                    </div>
                )}
                ...
                <div style={{display:'flex', alignItems:'center', justifyContent:'center', gap:'15px', marginBottom:'15px', background:'#f8fafc', padding:'10px', borderRadius:'15px'}}>
                   <button onClick={() => setCantidades({...cantidades, [p.id]: Math.max(1, cant - 1)})} style={{width:'35px', height:'35px', borderRadius:'50%', border:'none', background:'white'}}>-</button>
                   <span style={{fontWeight:'900'}}>{cant}</span>
                   <button onClick={() => setCantidades({...cantidades, [p.id]: cant + 1})} style={{width:'35px', height:'35px', borderRadius:'50%', border:'none', background:MONO_NARANJA, color:'white'}}>+</button>
                </div>
                <button onClick={() => agregarAlCarrito(p)} disabled={!p.disponible || !tiendaAbierta} style={{background: p.disponible ? MONO_NARANJA : '#ccc', color:'white', border:'none', padding:'15px', borderRadius:'15px', fontWeight:'900'}}>Añadir 🛒</button>
              </div>
            );
        })}
      </div>
      ...
      {pedido.length > 0 && (
        <div id="carrito_seccion" style={{ maxWidth: '750px', margin: '50px auto', background: 'white', padding: '30px', borderRadius: '40px', border: `5px solid ${MONO_NARANJA}` }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '20px' }}>
            <h2 style={{ fontWeight: '900' }}>🛒 Tu Pedido</h2>
            <button onClick={vaciarCarrito} style={{ background: '#fee2e2', color: '#dc2626', border: 'none', padding: '10px', borderRadius: '10px', fontWeight: 'bold' }}>🗑️ Vaciar Todo</button>
          </div>
          {pedido.map(item => (
            <div key={item.idUnico} style={{ display: 'flex', justifyContent: 'space-between', borderBottom: '1px solid #eee', padding: '10px 0' }}>
              ...
              <div><strong>${item.subtotal.toLocaleString()}</strong> <button onClick={() => eliminarDelCarrito(item.idUnico)} style={{background:'none', border:'none', color:'red', marginLeft:'10px'}}>❌</button></div>
            </div>
          ))}
          <div style={{marginTop: '20px', background: '#f8fafc', padding: '20px', borderRadius: '20px'}}>
            <h3 style={{margin: '0 0 10px 0'}}>🧂 Elige tus salsas:</h3>
          ...
          <h2 style={{ textAlign: 'right', color: MONO_NARANJA, fontSize: '36px', fontWeight: '900' }}>Total: ${pedido.reduce((acc, i) => acc + i.subtotal, 0).toLocaleString()}</h2>
          <div style={{ display: 'flex', flexDirection: 'column', gap: '15px' }}>
            ...
            <input type="text" placeholder="Dirección en Carepa" value={direccion} onChange={(e) => setDireccion(e.target.value)} style={{ padding: '15px', borderRadius: '15px', border: '1px solid #ddd' }} />
            <select value={metodoPago} onChange={(e) => setMetodoPago(e.target.value)} style={{ padding: '15px', borderRadius: '15px', border: '1px solid #ddd', fontWeight: 'bold' }}>
              <option value="">-- ¿Cómo pagas? --</option><option value="Efectivo">Efectivo</option><option value="Nequi">Nequi</option>
            </select>
            <button onClick={enviarWhatsApp} style={{ background: MONO_VERDE, color: 'white', border: 'none', padding: '20px', borderRadius: '20px', fontWeight: '900', fontSize: '20px' }}>WhatsApp 📲</button>
          </div>
        </div>
      )}
    </div>
