...
export default function App() {
  ...
  const [manualOverride, setManualOverride] = useState(null); // null = automático, true = abierto manual, false = cerrado manual
  ...
  // 🕒 Lógica de Horario con Override de Firebase
  useEffect(() => {
    const verificarEstado = () => {
      const hora = new Date().getHours();
      const estaEnHorario = hora >= 6 && hora < 11;
      
      // Si el dueño puso un cambio manual en Firebase, eso manda. Si no, manda el reloj.
  ...
  useEffect(() => {
    ...
    // Escuchar el interruptor maestro de Firebase
        ...
        <div style={{maxWidth:'850px', margin:'0 auto'}}>
          <div style={{display:'flex', justifyContent:'space-between', marginBottom:'30px', alignItems: 'center'}}>
            ...
            <div style={{display:'flex', gap:'10px', alignItems:'center'}}>
              <span style={{fontSize:'12px', fontWeight:'bold'}}>{tiendaAbierta ? 'TIENDA ABIERTA' : 'TIENDA CERRADA'}</span>
          ...
          <p style={{background:'#fff7ed', padding:'10px', borderRadius:'10px', fontSize:'13px', marginBottom:'20px'}}><b>Nota:</b> Si mueves el interruptor de arriba, obligas a la tienda a abrirse o cerrarse ignorando el horario automático.</p>
          
          {["Fritos", "Desayunos", "Arroces", "Bebidas"].map(cat => (
            <div key={cat} style={{background:'white', padding:'20px', borderRadius:'20px', marginBottom:'20px', boxShadow:'0 2px 5px rgba(0,0,0,0.05)'}}>
              <h3 style={{borderBottom:'1px solid #eee', paddingBottom:'5px'}}>{cat}</h3>
              {productosMostrar.filter(p => p.categoria === cat).map(p => (
                <div key={p.id} style={{padding:'10px 0', borderBottom:'1px solid #f9f9f9', display:'flex', justifyContent:'space-between', alignItems:'center'}}>
                   <span>{p.nombre}</span>
      ...
      {mostrarLogin && (
        <div style={{position:'fixed', top:0, left:0, width:'100%', height:'100%', background:'rgba(0,0,0,0.85)', zIndex:40000, display:'flex', justifyContent:'center', alignItems:'center'}}>
          <div style={{background:'white', padding:'30px', borderRadius:'30px', textAlign:'center', width:'90%', maxWidth:'350px'}}>
            <h3 style={{marginBottom:'20px'}}>Acceso de Propietario</h3>
            ...
            <button onClick={() => setMostrarLogin(false)} style={{marginTop:'15px', background:'none', border:'none', color:'#666', cursor:'pointer'}}>Volver</button>
          </div>
        </div>
      )}
      ...
      {agradecimiento && (
        <div style={{position:'fixed', top:0, left:0, width:'100%', height:'100%', background:MONO_NARANJA, zIndex:30000, display:'flex', justifyContent:'center', alignItems:'center', textAlign:'center', color:'white', padding:'20px'}}>
          <div>
            ...
            <h2 style={{fontSize:'32px', fontWeight:'900', margin:'20px 0'}}>¡Pedido en camino!</h2>
            <p style={{fontSize:'18px'}}>Ya recibimos tu orden en WhatsApp. <br/> El Mono está preparando todo.</p>
      ...
      {!tiendaAbierta && (
        <div style={{position:'fixed', top:0, left:0, width:'100%', height:'100%', background:'rgba(255,252,245,0.98)', zIndex:15000, display:'flex', justifyContent:'center', alignItems:'center', textAlign:'center', padding:'20px'}}>
          <div style={{display:'flex', flexDirection:'column', alignItems:'center'}}>
            ...
            {/* BOTÓN CLARO PARA ALBERTO */}
            <button 
              onClick={() => setMostrarLogin(true)} 
              style={{marginTop:'50px', background:'#eee', color:'#999', border:'none', padding:'10px 20px', borderRadius:'15px', fontSize:'12px', fontWeight:'bold', cursor:'pointer'}}
            >
              ACCESO PROPIETARIO
            </button>
          </div>
        ...
        {productosMostrar.filter(p => p.categoria === categoriaActiva).map(p => {
            ...
            return (
              <div key={p.id} className="card-mono" style={{background: 'white', borderRadius: '40px', padding: '20px', display: 'flex', flexDirection: 'column', boxShadow: '0 10px 20px rgba(0,0,0,0.02)', border:'1px solid #f1f5f9', opacity: p.disponible === false ? 0.6 : 1}}>
                ...
                <button onClick={() => agregarAlCarrito(p)} disabled={!tiendaAbierta || p.disponible === false} style={{background: (tiendaAbierta && p.disponible !== false) ? MONO_NARANJA : '#ccc', color:'white', border:'none', padding:'18px', borderRadius:'20px', fontWeight:'900', cursor:'pointer'}}>Añadir 🛒</button>
              </div>
            );
        })}
      </div>
      ...
      {pedido.length > 0 && (
        <div id="carrito_seccion" style={{ maxWidth: '750px', margin: '50px auto 100px', background: 'white', padding: '40px', borderRadius: '40px', border: `5px solid ${MONO_NARANJA}`, boxShadow: '0 20px 45px rgba(0,0,0,0.1)' }}>
          <div style={{display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:'20px'}}>
             ...
             <button onClick={vaciarCarrito} style={{background:'none', border:'none', color:'#ef4444', fontWeight:'bold', cursor:'pointer'}}>Vaciar</button>
          </div>
          {pedido.map(item => (
            <div key={item.idUnico} style={{ display: 'flex', justifyContent: 'space-between', borderBottom: '1px solid #eee', padding: '15px 0' }}>
              ...
              <div style={{display:'flex', alignItems:'center', gap:'10px'}}>
                ...
                <button onClick={() => eliminarDelCarrito(item.idUnico)} style={{background:'none', border:'none', color:'#ccc'}}>✕</button>
              </div>
            </div>
          ))}
